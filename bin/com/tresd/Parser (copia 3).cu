...
ves::= VES | {: RESULT = "ola q ase Keyly :D"; :} ;
...
puntocoma::= PUNTOCOMA | {: RESULT = "durmiendo o q ase Keyly"; :} ;
...
sent::= declasig PUNTOCOMA {: RESULT = "ola q ase"; :}
	| variable:var as:as valor:val PUNTOCOMA {:
		if(pasada() == 2)
	...
	| BREAK:v PUNTOCOMA {:
		if(pasada() == 2){
			if(idisplay >=0)
				...
				error("Instrucción break inválida", vleft + 1);
		}
	:}
	| ifelse
	| while
...
variable::= NEW ID:id PAREN vatrs:v TESIS {:
	...
	:}
	| THIS PUNTO ID COR dims CHETE
	| ID COR dims CHETE 
	| variable PUNTO ID COR dims CHETE ;
	
dims::= dims COMA valor 
	| valor ;
		...
		if(pasada() == 2)
			if(v != null)
				if(v.temp != null && v.tipo.equals(Variable.tboolean)){
					...
					addtif(v.temp, "0", display[idisplay]);
				} else
					error("Se espera expresión de condición", vleft + 1);
	:} TESIS sentencias {:
		if(pasada() == 2){
			addt("goto " + pop());
